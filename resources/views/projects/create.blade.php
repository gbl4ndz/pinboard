<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('projects.index') }}" class="text-stone-400 hover:text-stone-600 text-sm transition">← Projects</a>
            <span class="text-stone-300">/</span>
            <h2 class="font-semibold text-xl text-stone-800 leading-tight">New Project</h2>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('projects.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" type="text"
                               value="{{ old('name') }}"
                               required autofocus
                               class="w-full rounded-xl border border-stone-200 bg-stone-50 px-4 py-2.5 text-stone-800 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition @error('name') border-red-300 @enderror">
                        @error('name')
